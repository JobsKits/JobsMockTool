...
![image-20260602201811340](./assets/image-20260602201811340.png)
...
```text
dist/JobsMockTool.app
dist/JobsMockTool-Installer.dmg
```
...
```text
dist\JobsMockTool\JobsMockTool.exe
```
...
分发时请把整个 `dist\JobsMockTool` 文件夹一起发出去，不要只拷贝单独的 `JobsMockTool.exe`。
